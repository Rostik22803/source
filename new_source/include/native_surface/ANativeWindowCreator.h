...
// UPDATE 2026/07/31: логируем каждый неудачный resolve через logcat + printf, чтобы
// при следующем краше в /sdcard/Download был точный список отсутствующих символов.
#define ResolveMethod(ClassName, MethodName, Handle, MethodSignature)                                                                    \
    ...
    {                                                                                                                                    \
        __android_log_print(ANDROID_LOG_ERROR, "EclipsOxide", "[ANWC] dlsym miss: %s (%s::%s)", MethodSignature, #ClassName, #MethodName); \
        fprintf(stderr, "[ANWC] dlsym miss: %s (%s::%s)\n", MethodSignature, #ClassName, #MethodName);                                   \
    }
    ...
    namespace detail
    {
        ...
        struct Functionals
        {
            ...
            Functionals(const SymbolMethod &symbolMethod)
            {
                ...
                // UPDATE 2026/07/31 (Android 16 fix): для API 34+ LayerMetadata переехал
                // в namespace android::gui::. Ранее patchesTable ловил это только для
                // systemVersion == 14 → на Android 15/16/17 старый символ
                // "_ZN7android13LayerMetadataC2Ev" отсутствует в libgui.so → dlsym
                // возвращает nullptr → SIGSEGV на первом вызове конструктора.
                // Дублируем те же патчи для 15/16/17/18 (пока Google не поменял ABI ещё раз).
